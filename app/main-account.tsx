...
export default function MainAccountScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === "in")
      return payments.filter(
        (p) => p.beneficiaryAccount === accountInfo?.accountNumber,
      );
    if (tab === "out")
      return payments.filter(
        (p) => p.senderAccount === accountInfo?.accountNumber,
      );
    return payments;
  ...
  const groups = useMemo(() => {
    ...
    return Object.entries(byMonth)
      ...
      .sort(
        (a, b) =>
          new Date(b.label + " 1").getTime() -
          new Date(a.label + " 1").getTime(),
      );
  }, [filtered]);

  const currency = (a: any) => {
    const formatted = parseFloat(String(a || 0))
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    return `R${formatted}`;
  };
