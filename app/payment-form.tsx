...
import CapitecSelfieModal from "@/components/ui/CapitecSelfieModal";
...
export default function PaymentFormScreen() {
  ...
  const form = useMemo(
    () => beneficiaries.find((b) => b.account === params?.account),
    [beneficiaries, params?.account],
  );
  ...
  const [paymentType, setPaymentType] = useState<"normal" | "immediate">(
    "normal",
  );
  ...
  const calculateFee = (bankName: string, isImmediate: boolean = false) => {
    ...
    if (isImmediate && !isCapitec) {
      return 6.0; // R6 for immediate payments to other banks
    } else if (isCapitec) {
      return 1.0; // R1 for Capitec payments
    } else {
      return 2.0; // R2 for other banks
    }
  };
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      if (!capitecAllowed && isCapitec) {
        router.push({
          pathname: "/error",
          params: { message: capitecErrorMsg },
        });
        dispatch(setLoadingState({ isloading: false, type: "spinner" }));
        return;
      }
      if (!immediateAllowed && paymentType === "immediate") {
        router.push({
          pathname: "/error",
          params: { message: immediateErrorMsg },
        });
        dispatch(setLoadingState({ isloading: false, type: "spinner" }));
        return;
      }
      console.log(accountInfo?.immediateAmount, paymentAmount);
      if (immediateAllowed && paymentType === "immediate" && useRealMoney) {
        const immediateAmount = accountInfo?.immediateAmount || 0;
        if (immediateAmount < paymentAmount) {
          router.push({
            pathname: "/error",
            params: { message: useRealMoneyErrorMsg },
          });
          dispatch(setLoadingState({ isloading: false, type: "spinner" }));
          return;
        }
      ...
      const paymentData = {
        ...
        accountsMerged: [
          `${accountInfo?.accountNumber}_${accountNumber}`,
          `${accountNumber}_${accountInfo?.accountNumber}`,
        ],
      ...
      const feeData = {
        ...
        senderAccount: accountInfo?.accountNumber,
        accountsMerged: [
          `${accountInfo?.accountNumber}_1234567890`,
          `1234567890_${accountInfo?.accountNumber}`,
        ],
      ...
      if (success) {
        ...
        const updatedBeneficiaries = beneficiaries.map((b) =>
          b.id === form?.id
            ? {
                ...b,
                lastPaid: new Date(lastPaidTimestamp).toLocaleDateString(
                  "en-GB",
        ...
        await updateTable("beneficiaries", form?.account || "", {
          lastPaid: lastPaidTimestamp,
        });
        await updateTable("users", (accountInfo?.id as any) || "", {
          balance: newBalance,
          immediateAmount:
            (accountInfo?.immediateAmount || paymentAmount) - paymentAmount,
        });

        // Hard refresh account info from backend to ensure consistency
        try {
          const refreshed = await loginApi(
        ...
        schedulePushNotification(
          `Payment of ${currencyFormatter(paymentAmount)} to ${beneficiaryName} has been completed.`,
          "Payment completed",
        );
  ...
  useEffect(() => {
    const initialReference =
      `${accountInfo?.firstName?.slice(0, 1)} ${accountInfo?.lastName}` || "";
          ...
          <View style={styles.tabsRow}>
            ...
            >
              <Text
                style={[
                  styles.tabText,
                  activeTab === "payment" && styles.tabActiveText,
                ]}
              >
                Payment
              </Text>
          ...
          <>
            ...
            <View style={styles.card}>
              <TextField
                ...
                value={
                  form?.notificationType === "none"
                    ? "None"
                    : form?.notificationType?.toUpperCase() || "None"
                }
                onChangeText={() => {}}
                editable={false}
                rightChevron
                onPressRight={() =>
                  router.push({
                    pathname: "/payment-notification",
                    params: { account: form?.account } as any,
                  })
                }
              />
            </View>
                ...
                <LinearButton
                  ...
                  onPress={() => {
                    if (canPay) {
                      ...
                      if (parsedAmt >= 10000) {
                        setPendingPaymentType("immediate");
                        setShowSelfie(true);
                      } else {
